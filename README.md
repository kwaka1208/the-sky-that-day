...
- 天球儀では天の赤道・黄道・地平環・子午線環・極軸と、地平線下の天体もあわせて表示
- 天球儀の日周回転。時刻を進めて球が極軸まわりに回る様子を表示
- 星空部分のフルスクリーン表示
...
星図の見出し右の「星図／天球儀」で表示を切り替えます。天球儀は球体を外側から眺める表示のため、星座の並びは実際の空に対して左右が反転します。

- 極軸は観測地の緯度だけ傾き、天の北極と南極を結びます。球面には天の赤道（破線）と黄道を、球の外側には地平環と子午線環を描きます。
- 地平線より下の天体も表示し、その領域を暗く塗って区別します。
- ドラッグで視点の方位と仰角を変え、ホイールまたは＋−ボタンで拡大縮小、↺で初期視点に戻します。
- 球の下の操作バーで日周回転ができます。10分・1時間単位の前後送りと、1秒あたり1時間進む自動回転、入力した時刻に戻す操作があります。
- 日周回転は観測時刻そのものを動かします。見出しの日付と時刻は回転後の時刻を示し、日付をまたげば翌日に変わります。入力欄の日付・時刻は変わらず、操作バーには「＋2時間30分」のようにずれを表示します。日付または時刻を入力し直すと回転量は0に戻ります。
- 球の1回転は恒星日（約23時間56分）に対応します。
- 回転できるのは1900年から2100年の範囲までです。
- 回転中は過去天候と夜間光を取得しません。操作が止まってから取得し、限界等級へ反映します。
- 「地形遮蔽」をONにすると、地平環の内側に方位別の稜線を描きます。
- 天候の雲と光害のにじみは球体の見た目を損なうため描画しません。限界等級を通じた星の取捨には従来どおり反映されます。
- モバイルでコンパス追従中は全天星図に固定され、切り替えボタンは無効になります。
